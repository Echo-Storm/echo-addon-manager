...
int main(int argc, char** argv) {
    ...
    auto panel = [&]() {
        if (shotMode) {
            ...
            for (const char* h : { "Look (what the model does to the picture)", "Quality and performance", "Picture (sharpness, tone, colour, grain)", "Keep the HUD untouched", "Compare and hotkeys", "Games (a look per program)", "Frame detection (advanced)", "Technical status", "Advanced" })
