...
#   powershell -File deploy.ps1 -What host|nr|reshade|windowed|all [-StopLS] [-LsDir '<Lossless Scaling folder>']   (or set the LS_DIR environment variable) [-Game WowB]
param(
    [Parameter(Mandatory = $true)][ValidateSet('host', 'nr', 'reshade', 'windowed', 'all')][string]$What,
    [switch]$StopLS,
...
$items = @{
    ...
    reshade  = @{ Src = "$root\addons\LSP-ReShade\build\Release\LSP_ReShade.dll"; Dst = "$LsDir\addons\LSP-ReShade\LSP_ReShade.dll"; Extra = @("$root\addons\LSP-ReShade\addon.json") }
    windowed = @{ Src = "$root\addons\LSP-Windowed\build\LSP_Windowed\Release\LSP_Windowed.dll"; Dst = "$LsDir\addons\LSP-Windowed\LSP_Windowed.dll"; Extra = @("$root\addons\LSP-Windowed\addon.json") }
}
...
$names = if ($What -eq 'all') { @('host', 'nr', 'reshade', 'windowed') } else { @($What) }
foreach ($n in $names) {
    $it = $items[$n]
