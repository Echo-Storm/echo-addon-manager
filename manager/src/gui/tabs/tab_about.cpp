...
void RenderTabAbout() {
    ...
    ImGui::TextWrapped("This began as LosslessProxy by FrankBarretta (MIT), and we are grateful for it. Roughly half of this program's code "
                       "is still theirs: the part that loads into Lossless Scaling, hooks DirectX 11, finds and loads addons and passes "
                       "them events, and the ReShade and Windowed addons, which we fixed and reworked. The window, tabs, Performance, "
                       "backup, install and remove, the tray and the shared look were written since.");
    ImGui::Dummy(ImVec2(0, S(4)));
