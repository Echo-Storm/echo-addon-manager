...
Status: **0.1.0, the first release.** Built and tested against Lossless Scaling **3.2.2.0** (the official build) on Windows 11 with an RTX 4070 Ti SUPER; other
setups are untested.
...
Echo Addon Manager began as [LosslessProxy](https://github.com/FrankBarretta/LosslessProxy) by **FrankBarretta**, and we are grateful for it. About half
of the manager's code (the proxy DLL, the DirectX 11 and shader hooks, the addon loader and event system) is still theirs, and the ReShade and Windowed
addons started there too; everything you see in the window, and the live status and metrics, is new. Neural Rendering is by **andreiday**, extended
here. The full list, with licences, is in [NOTICE.md](NOTICE.md). Lossless Scaling belongs to its author; this project is unofficial.
